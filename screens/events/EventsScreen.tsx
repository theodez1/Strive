...
export default function EventsScreen() {
  ...
  const [pendingRequestsCounts, setPendingRequestsCounts] = useState<{[eventId: string]: number}>({});
  const [loading, setLoading] = useState(true);
  const [refreshing, setRefreshing] = useState(false);

  const fetchEvents = async (showLoader = true) => {
    try {
      // Ne lancer que si l'écran est focus et l'auth prête
      ...
      // Charger les demandes en attente pour les événements créés
      if (created && created.length > 0) {
        await loadPendingRequestsCounts(created);
      }
  ...
  const loadPendingRequestsCounts = async (events: EventWithDistance[]) => {
    try {
      const counts: {[eventId: string]: number} = {};
      
      for (const event of events) {
        const { data: pendingRequests } = await eventsService.getPendingRequestsByEvent(event.id);
        counts[event.id] = pendingRequests.length;
      }
  ...
  // Réinitialiser immédiatement quand on change de timeframe ou tab
  useEffect(() => {
    setCreatedEvents([]);
    setJoinedEvents([]);
    setLoading(true);
  }, [selectedTimeframe, selectedTab]);
  ...
  useFocusEffect(
    useCallback(() => {
      // Rafraîchissement silencieux au focus si auth prête
  ...
  const handleEventPress = (event: EventWithDistance) => {
    if (selectedTab === 'created') {
      // Navigation vers la gestion d'événement
      (navigation as any).navigate('EventAdmin', { eventId: event.id });
    } else {
      // Navigation vers les détails de participation
    ...
    return (
      <View style={isFirstItem ? { paddingTop: 10 } : null}>
        <EventCardMyevent
          ...
          pendingRequestsCount={pendingRequestsCounts[item.id] || 0}
        />
      </View>
    );
  };
